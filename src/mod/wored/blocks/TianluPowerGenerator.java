...
public class TianluPowerGenerator extends PowerDistributor {
    ...
    public TianluPowerGenerator(String name){
        super(name);
        sync = true;
        baseExplosiveness = 5f;
        flags = EnumSet.of(BlockFlag.generator);
        Attribute_Efficiency = false;//是否根据物品部分属性修改效率 true开启  false关闭
        Custom_attribute = 1;
    }
            ...
            if (item != null) {
                if (Attribute_Efficiency) {
                    if (Custom_attribute == 1) {
                        ...
                        Output_power = enabled ? powerProduction * itemAttribute * productionEfficiency : 0f;
                    }
                    ...
                    if (Custom_attribute > 5 || Custom_attribute < 1) {
                        Output_power = enabled ? powerProduction * productionEfficiency : 0f;
                    }
                }
            }
            return Output_power;
